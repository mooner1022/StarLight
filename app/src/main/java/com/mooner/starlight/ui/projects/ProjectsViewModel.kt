...
class ProjectsViewModel : ViewModel() {
    ...
    init {
        ...
        _data.value = projectLoader.loadProjects(true).map { ProjectCardData(it.config.name, getLanguageManager().getLanguage(it.config.language)!!, it.config.isEnabled, File(it.directory, it.config.mainScript)) }.toMutableList()
    }
}