/*
 * PluginLoader.kt created by Minki Moon(mooner1022) on 22. 1. 8. 오후 7:23
    ...
    fun loadPlugins(dir: File = defDirectory, onPluginLoad: ((name: String) -> Unit)? = null): Set<Plugin> {
        ...
        for ((file: File, info: PluginInfo) in pluginInfos.values) {
            try {
                ...
                if (info.apiVersion incompatibleWith dev.mooner.starlight.plugincore.Info.PLUGINCORE_VERSION) {
